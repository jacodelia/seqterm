...
/// A NoteOff event that should fire at a specific absolute step.
struct PendingNoteOff {
    dest_name: String,
    ch: u8,
    note: u8,
    at_step: usize,
    /// Clip key ("A0", "B3", etc.) for MPE channel release on NoteOff.
...
pub struct Scheduler {
    ...
    absolute_step: usize,
    /// NoteOffs scheduled by gate duration — fired lazily at the right step.
...
impl Scheduler {
    ...
    pub fn run(mut self) {
        ...
        loop {
            ...
            if elapsed >= tick_dur {
                ...
                // Send MIDI clock (0xF8) on every PPQN tick while playing.
    ...
    fn handle_command(&mut self, cmd: EngineCommand) {
        match cmd {
            EngineCommand::Play => {
                if self.midi_clock_out {
                    // MIDI Start (0xFA): reset clock to beat 1.
                    let msg = vec![0xFAu8];
                    self.clock_ports.retain(|tx| tx.send(msg.clone()).is_ok());
                }
            ...
            EngineCommand::Stop => {
                // Flush all pending NoteOffs to avoid stuck notes.
            ...
            EngineCommand::SwapProject(new_proj) => {
                ...
                // Reset pending note-offs so stale events don't fire against the new project.
            ...
            EngineCommand::SetAudioLatency { buffer_size, sample_rate } => {
                if sample_rate > 0 {
                    ...
                    self.audio_lookahead_steps =
                        (buffer_latency_ms / step_ms).ceil() as usize;
                    debug!(
    ...
    fn fire_all_clips(&mut self, global_step: usize) {
        // Flush pending NoteOffs whose step has arrived.
        let mut remaining = Vec::new();
        for noff in self.pending_note_offs.drain(..) {
            if noff.at_step <= self.absolute_step {
                if let Some(slot_str) = noff.dest_name.strip_prefix("__audio__") {
                    // Route NoteOff to audio engine SF2 slot.
                    ...
                    let _ = tx.send(vec![0x80 | noff.ch, noff.note, 0]);
                    // Release MPE channel after NoteOff.
                ...
                if let Some(note) = pat.steps.get(pos) {
                    if !note.is_empty() {
                        ...
                        match &clip.source {
                            ...
                            PatternSource::Sf2 { .. } => {
                                // Route MIDI events to the audio engine SF2 slot.
                                if let Some(slot_id) = audio_slot {
                                    ...
                                    for (midi_note, vel) in note.all_note_ons() {
                                        ...
                                        // Schedule NoteOff via pending queue (reuse same mechanism).
                                        let noff_step = self.absolute_step + gate_steps;
                        ...
                        for (midi_note, vel) in note.all_note_ons() {
                            // Determine the MIDI channel for this note.
                            let note_ch = if use_mpe {
                                // Ensure the MPE map exists for this clip.
                            ...
                            if let Some(ref tx) = midi_tx {
                                // Send per-note expressions before NoteOn.
