...
All FX implement `FxProcessor::process_block(&mut [f32], sample_rate)`.
Allocations happen only at construction; the audio path is zero-alloc.

| Processor  | Algorithm                                          |
|------------|----------------------------------------------------|
| Bitcrusher | Bit-depth quantisation + sample-hold decimation    |
| Svf        | Topology-preserving state-variable filter (Simper) |
| DelayLine  | Stereo delay with feedback + 1-pole LP damping     |
| VinylSim   | LFO wow/flutter + LCG crackle noise               |
| Reverb     | Freeverb: 8 comb filters + 4 allpass (stereo)      |
...
run_app() loop
  ...
  │     ├── draw_transport_bar
  │     ├── match app.view { Matrix | Tracker | Arranger | Mixer | Config | Sampler }
